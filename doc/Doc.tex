...
\usepackage[sorting=ydnt,bibstyle=alphabetic,url=false,doi=false]{biblatex}
...
\begin{figure}[h]
	\centering
	\centerline{\includegraphics[width=1.\linewidth]{VizuRegions.png}}
	\caption{Example of the Visualization Window with Regions}
	\label{vizuregions}
\end{figure}

%
%%%%%%%%%%%%%%%%%%%%%
%\chapter{Advanced explanations}
%\section{Segmentation}
%\subsection{MIDI Slicing}\label{slicing}
%Since MIDI data is already constituted of symbolic information (notes, velocities, channels\dots), the detection stage described~\ref{overview} is reduced at its minimal form in the MIDI part of OMax: getting the data stream on the correct port (and possibly select the channel to listen to). However, it is still necessary to segment very carefully this data stream to create consistent units and use a proper description to compare them. The segmentation of MIDI data is based on the appearance of new events and termination of past ones defining the beginning and end of what we call slices (Figure~\ref{slices}). Every slice has a duration information, and contains a series of channels, each of which contains pitch and velocity information and possibly other musical parameters.
%\begin{figure}[h]
%	\centering
%	\includegraphics[width=\linewidth]{Slices.png}
%	\caption{Segmentation of MIDI data into slices}
%	\label{slices}
%\end{figure}
%
%\begin{wrapfigure}{R}{0.6\linewidth}
%	\centering
%	\includegraphics[scale=0.2]{slicer-unslicer}
%	\caption{Slicing and unslicing of MIDI data}
%	\label{slicing}
%\end{wrapfigure}
%The slices are the units learnt in the knowledge model of OMax in MIDI mode. When improvising, they are recombined to recreate a polyphonic discourse (in MIDI format). This operation is name unslicing (Figure~\ref{slicing}) and has to be done even more cautiously. Indeed, the variations happening in OMax may create splits of notes, notes without any explicit end or other cases which need to be handled.
%
%
%\subsection{Virtual fundamental}
%
%
%\subsection{Pitch statistical analysis}
%
%
%\subsection{Spectral clustering}\label{specdetails}
%
%
%%
%\section{Model}
